...
def prepare_model(data_shape):
    ...
    z = tf.matmul(x,W) + b
    y = tf.sigmoid(z)
    loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=y_true, logits=z)) + 10 * tf.nn.l2_loss(W)
...
loss, X, Y, Y_true, W, b = prepare_model(data.shape)
